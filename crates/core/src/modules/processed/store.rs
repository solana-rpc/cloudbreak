...
//! one block per slot, prunes, and selects the view.
...
pub(crate) struct BlockStore {
    ...
    /// No view is served until the confirmed slot passes this slot.
...
impl BlockStore {
    ...
    /// The view from the highest stored slot, when its walk reaches the
    /// confirmed slot and blockhash with no poisoned slot on the way.
    pub(crate) fn select_view(&self) -> Option<ProcessedView> {
        ...
        chain.extend(std::iter::successors(at_anchor, |b| self.parent_of(b)).cloned());
...
pub(crate) mod tests {
    ...
    impl TestChain {
        ...
        pub fn event(&mut self) -> Option<ProcessedView> {
            self.store.prune();
            self.store.select_view()
        }
    ...
    fn linear_chain_serves_the_highest_slot_down_to_the_confirmed_block() {
        ...
        assert_eq!(view.chain.len(), 6);
        assert_eq!(view.chain.last().unwrap().slot, 100);
    }
    ...
    #[test]
    fn no_view_before_the_first_anchor() {
        let mut chain = TestChain::new();
        chain.linear(101, 103);
        assert_eq!(chain.head(), None);
        chain.store.set_anchor(anchor_at(100));
    ...
    fn retention_keeps_a_window_below_the_confirmed_slot() {
        ...
        let view = chain.event().unwrap();
        assert_eq!(view.chain.last().unwrap().slot, floor);
    }
