...
Maintains the running total-supply figure served by `getSupply` from a bounded hot-accounts cache. Requires an empty `[programs]` filter, `accounts-owner-map-enabled = false`, owner partitioning off, the `[snapshot]` section, and the stake-owner partial indexes. `from_config` validates all of it against the config and the database catalog and panics on any unmet requirement. Run on a dedicated supply-only instance. See [Supply](#supply-getsupply).

| Key                   | Type    | Default     | Description                                                                                     |
| --------------------- | ------- | ----------- | ----------------------------------------------------------------------------------------------- |
| `enabled`             | `bool`  | `false`     | Enables the supply tracker.                                                                      |
| `hot-accounts`        | `usize` | `1000000`   | Unpinned accounts kept resident in the hot-accounts cache (~64 bytes each), plus stake accounts. |
| `pin-stake-accounts`  | `bool`  | `true`      | Keep every stake account resident so epoch rewards are cache hits.                               |
...
`getSupply` is optional. It needs a **full, unfiltered index** (empty `[programs]` include and exclude lists), `accounts-owner-map-enabled = false`, owner partitioning off (single `accounts` and `snapshot_accounts` tables), the `[snapshot]` section, and the stake-owner partial indexes. Enable the `[supply]` section on the indexer and the `[supply]` section on the API. The two must be set consistently; a node with the API section absent or disabled answers `getSupply` with method-not-found. Run it on a dedicated supply-only instance: de-partitioning removes the owner pruning that `getProgramAccounts` relies on.

The indexer seeds the total supply from the snapshot bank's `capitalization` on every (re)start. The previous balance for a block's delta comes from a bounded in-memory hot-accounts cache keyed by pubkey, holding the last lamports and a `(slot, write_version)` stamp. For every account a confirmed block touches, a cache hit subtracts the stored lamports from the new balance in memory; a miss falls to one batched by-pubkey read against the de-partitioned tables, with no owner routing. Every stake account is pinned resident so the once-per-epoch reward burst stays cache hits. The cache is bounded: unpinned entries are capped (`hot-accounts`, default 1,000,000) and swept by a slot-window off the block path; the measured footprint is about 272 MB, against the ~185 GiB of the owner-map delta. While the snapshot is still ingesting, accounts touched by live blocks are recorded in a separate unbounded touched-account set; once the snapshot pass completes, those touches are reconciled against the startup-slot balances read back by pubkey from `snapshot_accounts` and the tracker goes live. One `supply` row carrying the total and the non-circulating lamports is persisted per confirmed block (pruned to the last 128 slots). A background recomputer derives the non-circulating set from Agave's pinned non-circulating account and withdraw-authority lists plus locked stake accounts, refreshing on the same converge-then-heartbeat cadence as the epoch-stakes recomputer; its stake scan also feeds the pinned set and rides the stake-owner partial index.

The API serves from a polled cache. Requests return a node-unhealthy error until the bootstrap pass has completed and the non-circulating set has been computed. On a detected slot gap the tracker keeps applying block deltas but stops publishing totals until every gap slot has been repaired (requests fail as node-unhealthy in the meantime). A failed account write while live pins the block's touched accounts so the cache stays authoritative and the total stays exact; if the failure-pin budget is exceeded the tracker marks stale and stays fail-closed until the indexer restarts. For `finalized` commitment the response picks the newest persisted total at or below the finalized slot; `confirmed` returns the newest total. `context.slot` is the slot of the served row; the non-circulating account list may lag it by up to the recomputer heartbeat.
...
The tracker is seeded from the startup snapshot pass and then advanced per confirmed block. It keeps accounts-per-mint holders in memory (default 100) so that evictions from the top 20 can be backfilled without rescanning. If churn ever exhausts that reservoir, the affected mint is marked stale and its persisted rows are dropped rather than serving a possibly-wrong answer. The non-circulating membership set behind the filter option comes from a background recomputer. It derives the set from Agave's pinned non-circulating account and withdraw-authority lists plus locked stake accounts, refreshing on a converge-then-heartbeat cadence.
...
| `cloudbreak_supply_cache_entries`                    | IntGaugeVec       | `kind`        | Hot-accounts cache population, labelled `pinned` (stake) and `hot` (unpinned).                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `cloudbreak_supply_cache_buckets`                    | IntGauge          | —             | Hot-accounts cache allocated bucket count.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `cloudbreak_supply_delta_seconds`                    | Histogram         | —             | Per-block supply delta time (probes, miss read, write-backs).                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `cloudbreak_supply_miss_read_seconds`                | Histogram         | —             | Per-block batched by-pubkey miss read time.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
