...
| `getTokenLargestAccounts`    | Returns the 20 largest token accounts of a mint. Mints listed in the indexer's `[largest-accounts].tracked-mints` are served from a maintained top-N record; other mints fall back to a live SQL scan. See [Largest Accounts](#largest-accounts-getlargestaccounts-gettokenlargestaccounts). |
...
| `getLargestAccounts`         | Returns the 20 largest accounts by lamport balance, with `filter: circulating\|nonCirculating` support. Optional; requires `[largest-accounts]` enabled on the indexer. See [Largest Accounts](#largest-accounts-getlargestaccounts-gettokenlargestaccounts). |
...
When enabled, the indexer keeps an in-memory top-N per tracked entity: the native-SOL top (backing plain `getLargestAccounts`), the circulating and non-circulating class tops (backing its `filter: circulating|nonCirculating` option), and one token top per mint listed in `tracked-mints`. The tracked-mints list is a *hotmap*: pick the mints whose `getTokenLargestAccounts` traffic matters — tracked mints are answered from the maintained record, while any other indexed mint transparently falls back to a live SQL scan. Whenever a top-N changes, its top 20 rows are persisted as a single packed record per (mint, slot); reads pick the newest record at or below the requested commitment's slot, and old generations are pruned every `prune-interval-slots` finalized slots, off the finalize critical path.

The tracker is seeded from the startup snapshot pass and then advanced per confirmed block. It keeps `accounts-per-mint` holders in memory (default 100) so that evictions from the top 20 can be backfilled without rescanning; if churn ever exhausts that reservoir the affected mint is marked stale and its persisted rows are dropped rather than serving a possibly-wrong answer (token mints then fall back to the SQL scan). The `filter: circulating|nonCirculating` option is served from the same feature: a background recomputer derives the non-circulating set from Agave's pinned non-circulating account and withdraw-authority lists plus locked stake accounts (refreshing on a converge-then-heartbeat cadence), and the indexer maintains circulating and non-circulating class tops alongside the SOL top. Because the filter routes balance lookups by owner, `[largest-accounts]` requires an empty `[programs]` filter, the `[snapshot]` section, and `accounts-owner-map-enabled = true`.

Config keys under `[largest-accounts]` (indexer): `enabled`, `tracked-mints` (array of mint addresses), `accounts-per-mint` (default 100), `prune-interval-slots` (default 20). See `example.cloudbreak.index.toml`.
...
| `cloudbreak_largest_accounts_db_errors`              | Counter           | —             | Largest-accounts DB write/prune errors (record persist, stale/cleared-mint deletes, and the pruner). Only emitted when `[largest-accounts]` is enabled.                                                                                                                                                                                                                                                                                                                                                       |
| `cloudbreak_largest_accounts_stale_mints`            | IntGauge          | —             | Number of tracked mints currently marked stale in the largest-accounts tracker (reservoir exhausted or a persist failure dropped the record). A non-zero value means those mints fall back to a live SQL scan. Only emitted when `[largest-accounts]` is enabled.                                                                                                                                                                                                                                             |
