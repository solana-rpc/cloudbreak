...
The two methods have different prerequisites, which is why they are enabled separately. getLargestAccounts needs a full unfiltered index, the [snapshot] section, and accounts-owner-map-enabled = true. Its filter option (circulating or nonCirculating) routes balance lookups by owner, which is what the owner map is for. getTokenLargestAccounts needs the token programs (Tokenkeg / Token-2022) indexed, because the mint-metadata lookup and the reservoir read token accounts. That means an unfiltered [programs], or an include-list containing both. It also needs the [snapshot] section. It does not need the owner map, the non-circulating tracker, or the GLA sentinels, so a token-filtered node can serve token tops alone.
