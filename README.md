...
| `getSupply`                  | Returns the total and circulating supply in lamports plus the non-circulating account list. Optional; requires `supply-tracker-enabled = true` on the indexer and `supply-enabled = true` on the API. See [Supply](#supply-getsupply). |
...
| `supply-tracker-enabled`     | `bool`  | `false` | Maintains the running total-supply figure served by `getSupply`. Requires an empty `[programs]` filter, `accounts-owner-map-enabled = true`, and the `[snapshot]` section. See [Supply](#supply-getsupply). |
...
#### `supply-enabled` (top-level, optional)

Serves `getSupply` from the supply ring the indexer maintains (default `false`). Set it together with the indexer's `supply-tracker-enabled`; a node with the key unset answers `getSupply` with method-not-found. See [Supply](#supply-getsupply).
...
`getSupply` is optional. It needs a **full, unfiltered index** (empty `[programs]` include and exclude lists), `accounts-owner-map-enabled = true`, and the `[snapshot]` section, with `supply-tracker-enabled = true` on the indexer and `supply-enabled = true` on the API. The two keys must be set consistently; a node with `supply-enabled = false` answers `getSupply` with method-not-found.

The indexer seeds the total supply from the snapshot bank's `capitalization` on every (re)start. Each account owner map entry also carries the account's last known lamports, so a block's supply delta is computed fully in memory: for every account updated in a confirmed block, the tracker subtracts the stored lamports from the new balance and folds the sum onto the running total. While the snapshot is still ingesting, accounts touched by live blocks are recorded in a small touched-account map; once the snapshot pass completes, those touches are reconciled against the startup-slot balances read back from the database and the tracker goes live. One `supply` row carrying the total and the non-circulating lamports is persisted per confirmed block (pruned to the last 128 slots). A background recomputer derives the non-circulating set from Agave's pinned non-circulating account and withdraw-authority lists plus locked stake accounts, refreshing on the same converge-then-heartbeat cadence as the epoch-stakes recomputer.

The API serves from a polled cache. Requests return a node-unhealthy error until the bootstrap pass has completed and the non-circulating set has been computed. On a detected slot gap the tracker keeps applying block deltas but stops publishing totals until every gap slot has been repaired (requests fail as node-unhealthy in the meantime); a failed account write marks it stale, and it stays fail-closed until the indexer restarts. For `finalized` commitment the response picks the newest persisted total at or below the finalized slot; `confirmed` returns the newest total. `context.slot` is the slot of the served row; the non-circulating account list may lag it by up to the recomputer heartbeat.
...
| `cloudbreak_supply_total_lamports`                   | IntGauge          | —             | Running total supply in lamports, updated on every committed block while the supply tracker is live. Only emitted when `supply-tracker-enabled` is set.                                                                                                                                                                                                                                                                                                                                                      |
| `cloudbreak_supply_slot`                             | IntGauge          | —             | Slot of the last committed supply total.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `cloudbreak_supply_stale`                            | IntGauge          | —             | `1` while the supply tracker is gap-filling or stale (totals are not being published), `0` while it is live.                                                                                                                                                                                                                                                                                                                                                                                                 |
