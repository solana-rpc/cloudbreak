...
| `getTokenLargestAccounts`    | Returns the 20 largest token accounts of a mint. Optional; requires `[token-largest-accounts]` on both indexer and API. Only mints in the indexer's `tracked-mints` are served, from a maintained top-N record; other mints get a fast "mint not tracked" error. See [Largest Accounts](#largest-accounts-getlargestaccounts-gettokenlargestaccounts). |
...
| `crates/core/`              | `cloudbreak-core`          | Shared library. Contains all configuration struct definitions (TOML deserialization), database connection helpers, the `AccountOwnerMap` abstraction, the `AccountSelectorConfig` (include/exclude program filter logic synced to DB), the shared `service_health` writer/reader, cross-crate metrics, and the self-contained `getLargestAccounts` and `getSupply` feature modules (`largest_accounts` top-N tracker with persistence and pruning, `non_circulating` stake map and real-time membership, and `supply` running total). The indexer drives these through a few thin calls.                                                                                                                 |
...
| `cleanup-interval-slots`     | `u64`   | `1`     | Finalized slots between cleanup drains. `1` drains every slot. Higher values coalesce repeat touches of an account into one delete, at the cost of extra account versions every latest-version read scans past. |

#### `[supply]` (optional)

Maintains the running total-supply figure served by `getSupply` from the non-circulating stake map and a bounded hot-accounts cache. Requires an empty `[programs]` filter, `accounts-owner-map-enabled = false`, owner partitioning off, and the `[snapshot]` section. `from_config` validates all of it against the config and the database catalog and panics on any unmet requirement. Run on a dedicated supply-only instance. See [Supply](#supply-getsupply).

| Key                   | Type    | Default     | Description                                                                                     |
| --------------------- | ------- | ----------- | ----------------------------------------------------------------------------------------------- |
| `enabled`             | `bool`  | `false`     | Enables the supply tracker.                                                                      |
| `hot-accounts`        | `usize` | `1000000`   | Accounts kept resident in the hot-accounts cache (~65 bytes each). Stake accounts live in the non-circulating stake map instead. |
...
Serves `getSupply` from the supply ring the indexer maintains. The section carries only `enabled` (default `false`). Set it together with the indexer's `[supply]` section; a node with the section absent or disabled answers `getSupply` with method-not-found. See [Supply](#supply-getsupply).
...
| `indexer-cleanup-lag-threshold` | `u64`         | `32`         | `cloudbreak_cleanup_lag_slots` threshold above which DDL is deferred. An indexer that does not publish the gauge reads as no pressure.     |
...
5. Before creating (or dropping) an index, the query tracker checks two indexer gauges: `cloudbreak_finalize_slot_handler_queue_size` against `indexer-metrics-threshold`, and `cloudbreak_cleanup_lag_slots` against `indexer-cleanup-lag-threshold`. Either one over its threshold defers DDL until the indexer catches up.
...
- `/debug/modules/supply` — the supply tracker status, total, slot, and hot-accounts cache counters, plus a single cache entry lookup.
- `/debug/modules/non_circulating` — the non-circulating tracker status, clock, member count, stake map size, running non-circulating lamports, the heads of the two lockup-expiry sets, plus a single stake map entry lookup.
...
The indexer seeds the total supply from the snapshot bank's `capitalization` on every (re)start. The previous balance for a block's delta comes from two in-memory sources, and each pubkey belongs to exactly one of them. Stake accounts belong to the non-circulating stake map, which holds every stake account and reports the summed stake delta per block, so the once-per-epoch reward burst is all memory reads. Every other account belongs to a bounded hot-accounts cache keyed by pubkey, holding the last lamports and the slot of the last write: a cache hit subtracts the stored lamports from the new balance in memory, and a miss falls to one batched by-pubkey read against the de-partitioned tables, with no owner routing, which a live block runs alongside its own row inserts. The cache is bounded: entries are capped (`hot-accounts`, default 1,000,000) and swept by a slot-window off the block path. The cache is about 136 MB and the stake map about 153 MB (both estimated from the record sizes), against the ~185 GiB of the owner-map delta. While the snapshot is still ingesting, accounts touched by live blocks are recorded in a separate unbounded touched-account set; once the snapshot pass completes, those touches are reconciled against the startup-slot balances read back by pubkey from `snapshot_accounts` and the tracker goes live. One `supply` row carrying the total and the running non-circulating lamports is persisted per confirmed block (pruned to the last 128 slots).

The non-circulating set is maintained in real time by the stake map. It is Agave's pinned non-circulating account list plus every stake account whose lockup is in force or whose withdrawer is a listed authority, evaluated against the Clock sysvar carried by each block. Members whose lockup can expire sit in two ordered sets keyed by their expiry epoch and expiry timestamp, and the head of each set is popped as the clock passes it, so a lockup expiry moves the account out of the set in the block whose clock reached it. The member list is persisted on the bootstrap flip and again on every block that changed it. Membership is evaluated against the confirmed block's clock while Agave evaluates against the bank clock at the requested commitment, so an account whose lockup expires inside the confirmed-to-finalized window is classified differently by the two for up to about 32 slots.

The API serves from a polled cache. Requests return a node-unhealthy error until the bootstrap pass has completed and the non-circulating set has been computed. On a detected slot gap the tracker keeps applying block deltas but stops publishing totals until every gap slot has been repaired (requests fail as node-unhealthy in the meantime). A failed account write while live pins the block's touched accounts so the cache stays authoritative and the total stays exact; if the failure-pin budget is exceeded the tracker marks stale and stays fail-closed until the indexer restarts. For `finalized` commitment the response picks the newest persisted total at or below the finalized slot; `confirmed` returns the newest total. `context.slot` is the slot of the served row; the non-circulating account list may lag it by one block.

The response matches Agave's `getSupply` exactly: the `RpcSupply` shape (`total`, `circulating`, `nonCirculating`, `nonCirculatingAccounts`), `total` is the bank capitalization, `nonCirculating` is the summed pinned-list-plus-locked-stake lamports, `circulating` is `total - nonCirculating`, and `excludeNonCirculatingAccountsList = true` returns the same numbers with an empty account list.

### Largest Accounts (`getLargestAccounts`, `getTokenLargestAccounts`)

These methods are served from **maintained top-N holder sets** rather than live table scans, so they stay cheap at any index size and work even where upstream RPC nodes disable them. Each method is enabled independently by its own config section: [largest-accounts] for getLargestAccounts, and [token-largest-accounts] for getTokenLargestAccounts. The section must be present with enabled = true on **both** the indexer and the API config of a co-deployed node. An absent section, or enabled = false, turns the method off. There is no shared enablement state in the database. A request for a method that is not enabled on the API gets a JSON-RPC -32601 Method not found.

The two methods have different prerequisites, which is why they are enabled separately. getLargestAccounts needs a full unfiltered index and the [snapshot] section. getTokenLargestAccounts needs the token programs (Tokenkeg / Token-2022) indexed, because the mint-metadata lookup and the reservoir read token accounts. That means an unfiltered [programs], or an include-list containing both. It also needs the [snapshot] section. It does not need the owner map, the non-circulating tracker, or the GLA sentinels, so a token-filtered node can serve token tops alone.

When enabled, the indexer keeps an in-memory top-N per tracked entity: the native-SOL top and the circulating/non-circulating class tops under [largest-accounts], and one token top per mint listed in tracked-mints. tracked-mints is the set of mints that get served, so pick the ones whose getTokenLargestAccounts traffic matters. When a top-N changes, its top 20 rows are persisted as a single packed record per (mint, slot). Reads pick the newest record at or below the requested commitment's slot. Old generations are pruned every prune-interval-slots finalized slots, off the finalize critical path.

The API routes getTokenLargestAccounts purely on **record presence**. The indexer persists a record for exactly the mints it tracks, so a persisted record is served and an absent record returns a fast "mint not tracked" error. This keeps the API in lockstep with the indexer without any mint-list handshake, and there is no fallback table scan. During the indexer's bootstrap window a tracked mint has no record yet, so it gets the same "mint not tracked" error until its first record persists. Clients cannot distinguish "warming up" from "never tracked" for getTokenLargestAccounts. getLargestAccounts with no record yet (the indexer is still bootstrapping) returns a retriable "not available yet" error instead of an internal error.

The tracker is seeded from the startup snapshot pass and then advanced per confirmed block. It keeps accounts-per-mint holders in memory (default 100) so that evictions from the top 20 can be backfilled without rescanning. If churn ever exhausts that reservoir, the affected mint is marked stale and its persisted rows are dropped rather than serving a possibly-wrong answer. The non-circulating membership set behind the filter option comes from the same real-time stake map that serves `getSupply` (see [Supply](#supply-getsupply)): each block's accounts are classified by the membership after that block, and a member whose lockup expired without a write is moved to the circulating top in the block whose clock passed the expiry.

Config keys: [largest-accounts] (indexer) takes enabled, accounts-per-mint (default 100), and prune-interval-slots (default 20). [token-largest-accounts] (indexer) takes enabled, tracked-mints (an array of mint addresses), accounts-per-mint, and prune-interval-slots. The API sections carry only enabled. See example.cloudbreak.index.toml and example.cloudbreak.api.toml.
...
| `/debug/modules/supply`       | GET    | Inspects the live supply tracker: status, total, slot, startup slot, gap-close count, and the hot-accounts cache counters (hot, pinned, capacity, cap, last sweep slot). Supports `?detail=full&pubkey=<base58>` for one cache entry. Returns `{"enabled": false}` when `[supply]` is not configured. See the rustdoc on `handle` in `crates/index/src/operational_endpoints/supply.rs`.                                                                                                                                                              |
| `/debug/modules/non_circulating` | GET | Inspects the live non-circulating tracker: status, slot, clock, member count, running non-circulating lamports, stake map size, the sizes and heads of the two lockup-expiry sets (`?limit=` heads each, default 10). Supports `?detail=full&pubkey=<base58>` for one stake map entry. Returns `{"enabled": false}` when neither `[largest-accounts]` nor `[supply]` is configured. See the rustdoc on `handle` in `crates/index/src/operational_endpoints/non_circulating.rs`.                                                                                        |
...
| `cloudbreak_current_tokio_tasks`                     | IntGauge          | `task_type`   | Current count of spawned Tokio tasks per category. `task_type` values: `grpc`, `slot_finalizer`, `self_healing_fill_gaps`, `snapshot_processing`, `insert_accounts_chunk`, `insert_closed_accounts`, `startup_snapshot_accounts_cleanup`, `finalize_cleanup_drainer`, `metrics_server`, `epoch_stakes_recomputer`, `largest_accounts_pruner`, `supply_cache_sweeper`. Counter is guarded by `TokioTaskCounterGuard` so panicking tasks still decrement.                                                                                                                                         |
...
| `cloudbreak_cleanup_lag_slots`                       | IntGauge          | —             | Finalized slots since the oldest cleanup key still waiting. Cleanup runs off the finalize worker, so this is the signal that it is falling behind, and the query tracker reads it as the second DDL gate (`indexer-cleanup-lag-threshold`).                                                                                                                                                                                                                                                                        |
| `cloudbreak_largest_accounts_db_errors`              | Counter           | —             | Largest-accounts DB write/prune errors (record persist, stale/cleared-mint deletes, and the pruner). Only emitted when a largest-accounts section (`[largest-accounts]` / `[token-largest-accounts]`) is enabled.                                                                                                                                                                                                                                                                                                                                                       |
| `cloudbreak_largest_accounts_stale_mints`            | IntGauge          | —             | Number of tracked mints currently marked stale in the largest-accounts tracker (reservoir exhausted or a persist failure dropped the record). A non-zero value means those mints have no persisted record and `getTokenLargestAccounts` fails fast for them. Only emitted when a largest-accounts section (`[largest-accounts]` / `[token-largest-accounts]`) is enabled.                                                                                                                                                                                                                                             |
| `cloudbreak_supply_total_lamports`                   | IntGauge          | —             | Running total supply in lamports, updated on every committed block while the supply tracker is live. Only emitted when the `[supply]` section is enabled.                                                                                                                                                                                                                                                                                                                                                     |
| `cloudbreak_supply_slot`                             | IntGauge          | —             | Slot of the last committed supply total.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `cloudbreak_supply_status`                           | IntGauge          | —             | Supply tracker status: `0` bootstrapping, `1` live, `2` gap filling, `3` stale, `4` bootstrap failed. Set at every transition.                                                                                                                                                                                                                                                                                                                                                                               |
| `cloudbreak_supply_cache_entries`                    | IntGaugeVec       | `kind`        | Hot-accounts cache population, labelled `pinned` (by a live write failure) and `hot` (unpinned).                                                                                                                                                                                                                                                                                                                                                                                                             |
| `cloudbreak_supply_cache_hits_total`                 | Counter           | —             | Per-block cache hits (previous balance served from memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `cloudbreak_supply_cache_misses_total`               | Counter           | —             | Per-block cache misses (previous balance resolved by a DB read).                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `cloudbreak_supply_write_failures_total`             | Counter           | —             | Blocks whose account writes failed while live (pinned on failure, or marked stale beyond the pin budget).                                                                                                                                                                                                                                                                                                                                                                                                   |
| `cloudbreak_supply_block_microseconds`               | Histogram         | —             | Per-block supply delta time in microseconds, from `apply_block` entry (block-writes lock wait included) to the last miss write-back.                                                                                                                                                                                                                                                                                                                                                                       |
| `cloudbreak_supply_probe_microseconds`               | Histogram         | —             | Per-block cache probe time in microseconds (phase one, under the state mutex).                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `cloudbreak_supply_db_microseconds`                  | HistogramVec      | `query`       | Supply DB query time in microseconds: `prev_balances` (the batched miss read), `persist_row` (the supply row upsert), `startup_balances` (the bootstrap resolve read).                                                                                                                                                                                                                                                                                                                                     |
| `cloudbreak_supply_sweep_microseconds`               | Histogram         | —             | One hot-accounts cache sweep pass in microseconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `cloudbreak_supply_query_errors`                     | Counter           | —             | Supply persistence queries that failed or timed out (the per-block supply-row upsert and the non-circulating list upsert).                                                                                                                                                                                                                                                                                                                                                                                   |
| `cloudbreak_non_circulating_block_microseconds`      | Histogram         | —             | Per-block non-circulating time in microseconds: the stake map apply plus the lockup-expiry pop. Only emitted when `[largest-accounts]` or `[supply]` is enabled.                                                                                                                                                                                                                                                                                                                                             |
| `cloudbreak_non_circulating_members`                 | IntGauge          | —             | Current non-circulating member count, set on the bootstrap flip and on every block that changed membership.                                                                                                                                                                                                                                                                                                                                                                                                  |
| `cloudbreak_non_circulating_changes_total`           | IntCounterVec     | `kind`        | Non-circulating membership changes, labelled `join` and `leave`.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
