...
| `getLargestAccounts`         | Returns the 20 largest accounts by lamport balance, with `filter: circulating\|nonCirculating` support. Optional; requires `[largest-accounts]` enabled on the indexer (the filter additionally requires supply support). See [Largest Accounts](#largest-accounts-getlargestaccounts-gettokenlargestaccounts). |
...
The tracker is seeded from the startup snapshot pass and then advanced per confirmed block. It keeps `accounts-per-mint` holders in memory (default 100) so that evictions from the top 20 can be backfilled without rescanning; if churn ever exhausts that reservoir the affected mint is marked stale and its persisted rows are dropped rather than serving a possibly-wrong answer (token mints then fall back to the SQL scan). Plain `getLargestAccounts` needs no supply support, but the `filter` option is only served when `getSupply` support is available (full unfiltered index with the supply tracker enabled), since circulating classification comes from the non-circulating set.
